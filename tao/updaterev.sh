...
# retrieved from Git
#
# Usage:  ./update_rev.sh [-n | name]
#   -n:    Display version string and exit. version.h is not modified.
#   name:  If present, define TAO_EDITION to "name" in version.h.
#
#
...
if [ "$1" ] ; then
  ...
  (echo "$1" | grep -qi "Discovery") && echo "#define TAO_EDITION_IS_DISCOVERY 1" >> .version.new;
else
